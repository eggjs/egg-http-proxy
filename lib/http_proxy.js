...
class HttpProxy {
   ...
   * @param {Function} [options.beforeResponse] - modify response
  ...
  async curl(host, options) {
    ...
    const defaultOptions = {
      ...
      beforeResponse: res => res,
      withCredentials: false,

      method: ctx.method,
    };

    if (typeof this.config.agent !== 'undefined') {
      defaultOptions.agent = this.config.agent;
    }
    options = {
      ...defaultOptions,
      ...options,
    ...
    if (ctx.method === 'POST' || ctx.method === 'PUT') {
      ...
      if (ctx.is('multipart') || ctx.is('application/octet-stream')) {
        ...
        // recommend: if you use `proxyRequest()` at middleware and before `bodyParser` and `multipart` then it will run this.
        options.stream = ctx.req;
      }
    }
    ...
    const { headers, status, data, res } = options.beforeResponse(proxyResult);
