...
class HttpProxy {
  ...
  async curl(host, options) {
    ...
    let cacheId;
    let hitCache;
    if (options.cache === true && this.config.cacheManager) {
      cacheId = this.config.cacheManager.calcId(targetUrl, options);
      if (cacheId) {
        proxyResult = this.config.cacheManager.get(cacheId);
        if (proxyResult) {
    ...
    if (!hitCache) {
      try {
        ...
        // store cache
        if (cacheId) {
          this.config.cacheManager.set(cacheId, proxyResult);
        }
      } catch (err) {
    ...
    let body = data || res;
    if (!Buffer.isBuffer(body) && typeof body !== 'string' && !(body instanceof Stream)) {
      // body: json
